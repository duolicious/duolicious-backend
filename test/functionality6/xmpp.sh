...
curl -X POST http://localhost:3001/config -H "Content-Type: application/json" -d '{
...
curl -X POST http://localhost:3001/send -H "Content-Type: application/xml" -d "
<duo_ping/>
"
...
curl -sX GET http://localhost:3001/pop | grep -qF '<duo_message_blocked id="id1"/>'
...
curl -sX GET http://localhost:3001/pop | grep -qF '<duo_message_delivered id="id1"/>'
...
curl -sX GET http://localhost:3001/pop | grep -qF '<duo_registration_successful />'
...
curl -sX GET http://localhost:3001/pop | grep -qF '<duo_message_not_unique id="id2"/>'
...
curl -sX GET http://localhost:3001/pop | grep -qF '<duo_message_delivered id="id3"/>'
...
curl -sX GET http://localhost:3001/pop | grep -qF '<failure xmlns="urn:ietf:params:xml:ns:xmpp-sasl"><not-authorized/></failure>'
...
curl -sX GET http://localhost:3001/pop | grep -qF '<body>hello user 2</body>'
