...
from urlslug import assign_url_slug, slug_base, RESERVED_SLUGS
    ...
    if field_name in ['name', 'date_of_birth']:
        ...
        with api_tx() as tx:
            ...
            if field_name == 'name':
                ...
                # sign-up.
                base = slug_base(field_value)

                available = bool(base) and base not in RESERVED_SLUGS
                if available:
                    available = not tx.execute(
                        "SELECT EXISTS (SELECT 1 FROM person WHERE url_slug = %(s)s)",
                        dict(s=base),
                    ).fetchone()['exists']

                return dict(base_slug=base, available=available)
